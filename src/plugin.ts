...
import { OPENAI_COMPATIBLE_NPM, PLACEHOLDER_MODEL_ID, PLEXUS_PROVIDER_ID, PLEXUS_PROVIDER_NAME, REFRESH_TTL_MS } from "./constants"
import { readCachedModels, readCachedModelsSync, writeCache } from "./cache"
import { resolveConfig, persistToGlobalConfig } from "./config-store"
import { createLogger } from "./log"
import { buildModels, type ConfigModel } from "./models"
import { fetchPlexusModels } from "./plexus-client"
import { apiBase, trimURL } from "./url"
...
export const PlexusProviderPlugin: Plugin = async (ctx) => {
  ...
  return {
    config: async (cfg) => {
      ...
      const existingOptions = (typeof existing["options"] === "object" && existing["options"] !== null
        ? existing["options"]
        : {}) as Record<string, unknown>
      const existingModels = (typeof existing["models"] === "object" && existing["models"] !== null
    ...
    auth: {
      ...
      async loader(getAuth, providerInfo) {
        ...
        const key =
          (auth?.type === "api" ? (auth as { type: string; key: string }).key : undefined) ?? apiKey
