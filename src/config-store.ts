...
import { ENV_API_KEY, ENV_BASE_URL, PLEXUS_PROVIDER_ID } from "./constants"
...
export function resolveConfig(provider?: ProviderConfig): { baseURL?: string; apiKey?: string } {
  ...
  const optApiKey =
    typeof provider?.options?.apiKey === "string" ? (provider.options.apiKey as string).trim() : undefined
