...
import type { PlexusApiResponse } from "./types"
...
/** Resolve the cache directory using `client.path.get()`, falling back to a homedir default. */
async function getDir(client: PluginInput["client"]): Promise<string> {
  if (resolvedDir) return resolvedDir

  try {
    const res = await client.path.get()
    // SDK returns { data: Path } via the generated client wrapper
...
export async function writeCache(
  ...
  try {
    ...
    // Also mirror to homedir path so the sync fallback is always fresh
    try {
